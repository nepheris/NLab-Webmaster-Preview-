...
import {recipeDetailV4,ingredientDetailV4,trialDetail} from './detail.js';

async function loadDefault(force=false){
  if(!force){const saved=loadSavedData();if(saved)return saved}
  const [mainR,trialsR]=await Promise.all([fetch('./data/demo.json',{cache:'no-store'}),fetch('./data/trials.json',{cache:'no-store'}).catch(()=>null)]);
  if(!mainR.ok)throw new Error(`data ${mainR.status}`);const d=await mainR.json();
...
export function render(){
  ...
  else if(state.detail){const {type,id}=state.detail,x=(state.data?.[type]||[]).find(o=>o.id===id);if(!x)app.innerHTML='<div class="panel">Objet introuvable.</div>';else if(type==='recipes')app.innerHTML=recipeDetailV4(x);else if(type==='ingredients')app.innerHTML=ingredientDetailV4(x);else if(type==='trials')app.innerHTML=trialDetail(x);else app.innerHTML=genericDetail(x)}
  else if(state.section==='home')app.innerHTML=home()+section('home-data','Données du prototype',dataControlsMarkup(),{open:false,helpShort:'Import/export local du JSON de démonstration.'});
  else if(state.section==='concept'||state.section==='legal')app.innerHTML=editorialPage(state.section);
...
async function init(){loadPrefs();state.data=await loadDefault();restoreFromUrl();shell();initSearch();initThemeUI();initHelp();initToolbar();initQr();document.addEventListener('cuisinex:search-change',()=>{state.page=1;render()});document.addEventListener('cuisinex:open',e=>{state.detail=e.detail;state.section=e.detail.type;state.page=1;state.searchContext='section';render();scrollTo({top:0,behavior:'smooth'})});document.addEventListener('cuisinex:open-qr',openQr);render()}
