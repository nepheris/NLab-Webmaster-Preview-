...
async function clarifyAuthState(){const panel=q('#v14GithubPanel'),toggle=q('#v15LoginToggle');if(!panel||!toggle)return;let cfg=null;try{const r=await fetch(`${AUTH_CFG}?_=${Date.now()}`,{cache:'no-store'});if(r.ok)cfg=await r.json()}catch{}const token=!!sessionStorage.getItem('nlab-preview-github-user-token');let note=q('.v16-auth-note',panel);if(!note){note=document.createElement('div');note.className='v16-auth-note';q('.v14-status-grid',panel)?.insertAdjacentElement('afterend',note)}if(token){note.innerHTML='<b>Authentification active.</b> La session peut tester l’accès au Data Store privé.';return}if(!cfg?.client_id){toggle.title='GitHub App non configurée : Client ID manquant';const label=toggle.querySelector('span:last-child');if(label)label.textContent='Connexion GitHub · Configuration requise';const state=q('#v14AuthState');if(state)state.textContent='Configuration requise';note.innerHTML='<b>Login non activable pour l’instant.</b> La GitHub App doit d’abord être enregistrée et son Client ID ajouté à la configuration. Aucun PAT ou token manuel n’est demandé.'}else{note.innerHTML='<b>GitHub App détectée.</b> Le Login nécessite la passerelle OAuth sécurisée pour échanger le code sans exposer le secret dans GitHub Pages.'}}
...
function loadV17(){if(document.querySelector('script[data-nlab-v17]'))return;const s=document.createElement('script');s.dataset.nlabV17='1';s.src='assets/preview-v17.js?v=1.7';s.defer=true;s.onerror=()=>console.error('nLab Preview: impossible de charger V1.7');document.head.appendChild(s)}
function init(){addStyles();updateVersion();installReviewCloseDemo();alignGithubDataButtons();clarifyAuthState();observeGallery();setTimeout(()=>{installReviewCloseDemo();alignGithubDataButtons();clarifyAuthState();compactVersionRows();loadV17()},500)}
